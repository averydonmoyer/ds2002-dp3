...
def get_message():
    try:
        ...
        if "Messages" in response:
            ...
            newdict = {"order":order, "word": word}
            riddle.append(newdict)
...
if __name__ == "__main__":
    # need some sort of loop to get all messages from the queue
    # Or they might get 10 messages in one call. But that would
    # be advanced.
    for i in range(10):
        get_message()

    # order riddle by the order key - need some kind of sorting
    riddle.sort(key=lambda x: x['order'])

    # print sorted riddle as a sentence.
    ans = ""
    for i in riddle:
        ans += i['word'] + " "
    print(ans)
